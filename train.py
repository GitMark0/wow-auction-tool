...
def plot_results(model, X, y):
    ...
    plt.subplot(211)
    plt.title('train_set')
    plt.plot(days_train, scaler.inverse_transform(y_train_.reshape(-1, 1)), label='predicted')
    plt.plot(days_train, scaler.inverse_transform(y.reshape(-1, 1)), label='truth')
    plt.legend()
    plt.subplot(212)
    plt.title('test_set')
    plt.plot(days_test, scaler.inverse_transform(y_test_.reshape(-1, 1)), label='predicted')
    plt.plot(days_test, scaler.inverse_transform(y_test.reshape(-1, 1)), label='truth')
    plt.legend()
    plt.show()
